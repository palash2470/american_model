            ...
            <div id="photo_{{$image->id}}" class="photo-list">
                <div class="row">
                    <div class="col-lg-5 col-md-6 col-sm-12 col-12">
                        <div class="popup-img">
                            <img class="img-block photo_Height masonry_pop_height" src="{{url('img/user/images/'.$image->image)}}">
                        </div>
                    </div>
                    <div class="col-lg-7 col-md-6 col-sm-12 col-12">
                        <div class="ftr-cmnt photo_Height masonry_pop_height">
                            <div class="own-user-wrap">
                                <div class="d-flex align-items-center">
                                    <div class="own-user-lft">
                                        <a href="{{url('/profile/'.$image->user->category->slug.'/'.$image->user->name_slug)}}" class="own-user-img">
                                            <img class="img-block" src="{{url('/img/user/profile-image/'.$image->user->userDetails->profile_image)}}" alt="">
                                        </a>
                                    </div>
                                    <div class="own-user-rgt">
                                        <a href="{{url('/profile/'.$image->user->category->slug.'/'.$image->user->name_slug)}}">{{$image->user->name}}</a>
                                    </div>
                                </div>
                                <p>{{$image->description}}</p>
                                <ul class="user-like">
                                    <li><a href="javascript:void(0)" onclick="photoLike({{$image->id}})"><i class="{{Auth::check() && $image->checkUserLike ? 'fas fa-thumbs-up' : 'far fa-thumbs-up'}}" id="like_icon_{{$image->id}}"></i></a><span id="image_like_{{$image->id}}">{{count($image->likes)}}</span></li>
                                </ul>
                            </div>
